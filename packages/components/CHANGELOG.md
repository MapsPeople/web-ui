...
- An issue when pipeline would throw build errors.
...
- **mi-chip**, **mi-route-instructions-step-header**: Added missing Eleventy data files (`chip.json`, `route-instructions-step-header.json`) to resolve a docs build permalink conflict.

## [13.33.5] - 2026-04-07

### Added

- **mi-route-instructions-step-header**: New component that renders a step heading and optional substep toggle, extracted from `mi-route-instructions-step` to allow independent control of the header.
- **mi-search**: Added `focused` event emitted when the input field receives focus.

### Changed

- **mi-route-instructions-step**: Added `showHeader` prop (default `true`) to allow hiding the built-in step heading when using `mi-route-instructions-step-header` externally.
- **mi-route-instructions-step**: Changed from `shadow: true` to `scoped: true` for consistent style inheritance.
- **mi-route-instructions-maneuver**: Changed from `shadow: true` to `scoped: true` for consistent style inheritance.

## [13.33.4] - 2026-03-19

### Fixed

- **mi-floor-selector**: changed floor selector max height.
- **mi-floor-selector**: Removed visible scrollbar in favour of fade gradient indicators, preventing layout shift within the 48px component width.
- **mi-floor-selector**: Fixed visual glitch when switching buildings while the floor selector is closed — in-flight animations are now cancelled and new floor positions snap into place instantly.

## [13.33.3] - 2026-03-05

### Changed

- Added `dist-custom-elements` output target with `customElementsExportBehavior: 'bundle'` to Stencil config for static component bundling
- Kept `dist` output target for backward compatibility with existing lazy-loaded consumers

## [13.33.2] - 2026-03-04

### Fixed

- Added nullguards to (`data-table`, `dropdown`, `floor-selector`, `keyboard`, `list`, `my-position`, `route-instructions`, `scroll-buttons`, `search`, `share-sms`) components
...
- Updated MapsIndoors SDK to version 4.52.0 across several test files.
...
- Improved styling when text size is 200% in list-item-location, location-info, and search components.
