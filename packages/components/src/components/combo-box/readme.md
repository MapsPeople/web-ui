...
| `includeValueInFilter` | `include-value-in-filter` | When set, the item's `value` is also matched by the filter input, in addition to its displayed text. Lets callers filter by hidden data (e.g. an id) without it appearing in the input or list. | `boolean`                         | `false`              |
