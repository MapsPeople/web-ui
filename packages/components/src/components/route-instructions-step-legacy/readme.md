# mi-route-instructions-step-legacy

The `<mi-route-instructions-step-legacy` element is used internally within the `<mi-route-instructions>` custom element, and shows route instructions for a MapsIndoors route step.

Working example:

<mi-route-instructions-step-legacy></mi-route-instructions-step-legacy>

<script>
const stepElement = document.querySelector('mi-route-instructions-step-legacy');
const step = {
     distance: {
        text: '',
        value: 24
...
<mi-route-instructions-step-legacy-legacy></mi-route-instructions-step-legacy-legacy>
```

```js
// JavaScript

const stepElement = document.querySelector('mi-route-instructions-step-legacy-legacy');
const step = {
     distance: {
        text: '',
        value: 24
...
A `step` attribute is available on the `<mi-route-instructions-step-legacy-legacy>` element which can be used to pass the step object as stringified JSON. The attribute is required.

## `hideIndoorSubsteps` attribute

A `hideIndoorSubsteps` attribute is available on the `<mi-route-instructions-step-legacy-legacy>` element. Set this to `false` if indoors substeps should be visible. Default is `true`.

## `fromRouteContext` attribute

A `fromRouteContext` attribute is available on the `<mi-route-instructions-step-legacy>` element which can be used to pass the route context of the previous step.

## `fromTravelMode` attribute

A `fromTravelMode` attribute is available on the `<mi-route-instructions-step-legacy>` element which can be used to pass the travel mode of the previous step.

## `fromRouteContext` attribute

A `fromRouteContext` attribute is available on the `<mi-route-instructions-step-legacy>` element which can be used to pass the transit stop of previous step.

## `translations` attribute

A `translations` attribute is available on the `<mi-route-instructions-step-legacy>` element which can be used to pass a object with translation strings as stringified JSON. The attribute is required.
...
A `unit` attribute is available on the `<mi-route-instructions-step-legacy>` element which can be used to control how distances are presented. Possible values are `metric` (which will show distances in meters and km) and `imperial` (which will show distances in feet and miles). Default value is `metric`.

## `stepClicked` event

A `stepClicked` event is emitted from the `<mi-route-instructions-step-legacy>` element when clicked.
...
mi-route-instructions-step-legacy::part(step-toggle) {
