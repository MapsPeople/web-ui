...
|`search-external-results`|`searchExternalResults`|`bool`|If you want to perform search for external results. If set to true, Mapbox/Google places will be displayed depending on the Map Provider you are using. If set to false, the results returned will only be MapsIndoors results. The default is false. |
