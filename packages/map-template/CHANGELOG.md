...
- Pitch and bearing keep decimal values when applied to the map, instead of being truncated to whole numbers.

## [1.99.28] - 2026-09-24

### Changed

- Updated the MapsIndoors Web SDK to version 4.61.2.
...
- Updated the MapsIndoors Web SDK to version 4.61.1.

## [1.99.25] - 2026-09-09

### Fixed

- Location selection pin no longer waits for Mapbox `idle`. That event is held until 3D models finish loading, which delayed the pin by several seconds and could leave two pins when clicking quickly.
...
- Updated the MapsIndoors Web SDK to version 4.59.4.

## [1.99.20] - 2026-08-26

### Changed

- Updated the MapsIndoors Web SDK to version 4.59.3.
...
- Chat-triggered directions honor a per-request `avoidStairs` override from the backend, without affecting the persistent accessibility toggle

### Changed

- Extracted `processFunctionData` out of `GeminiProvider` into its own testable module
...
- Updated to Web SDK v4.58.6

## [1.99.7] - 2026-06-24

### Fixed

- Analytics `viewVariant` is now set on the Web SDK Logger as soon as the SDK loads, using `kioskOriginLocationId` instead of the async kiosk location fetch, so bootstrap events (`SDK_LOADED`, `MAPSINDOORS_INSTANTIATED`, `MAP_INSTANTIATED`) are tagged as `kiosk` on kiosk hosts rather than `desktop`.
...
- Categories are now fetched using the venue id instead of the venue name, preventing requests with an incorrect venue identifier when names differ in casing.
...
- Kiosk QR Code visibility is now configurable via App Config object.
...
- Add send button to ChatInput with new SVG icon and styles

### Fixed

- Change expandedMessages state from Map to Set
- Update viewBox attribute in send.svg for proper alignment
- Refactor padding calculations for map bounds into separate functions
- Add primaryColor prop to ChatInput and pass it from ChatWindow

### Changed

- Rename handleChatSearchResults to handleChatLocations
...
### Added
- Full screen button (desktop only). Configurable via App Config `enableFullScreenButton` under `appSettings`. Can also be excluded via `excludeFromUI`.

## [1.96.0] - 2026-03-05

### Fixed
- Chat window AI usage consent overlay reappearing when navigating to the route view and back
...
- Remove dynamic imports for build-for-npm.

## [1.95.7] - 2026-03-04

### Added

- Add files paths to package.json in the Map Template project.

## [1.95.6] - 2026-03-04

### Added

- Add better docs to MapControls

## [1.95.5] - 2026-03-04

### Added

- Add built web components to map-template dist

## [1.95.4] - 2026-03-04

### Added

- Upgraded to Web SDK 4.52.2

## [1.95.3] - 2026-03-04

### Changed

- Bump components to use the new null guards

## [1.95.2] - 2026-03-04

### Changed

- Bump to use components v4
...
- Improved styling when text size is 200% in Directions, LocationDetails, RouteInstructions, Search, Categories, and Modal components.

## [1.93.0] - 2026-02-23

### Added

- Steps rendering is now configurable through the App Config.

## [1.92.4] - 2026-02-23

### Fixed

- Upgraded to Web SDK 4.50.0
...
- Merged `@mapsindoors/react-components` package into `@mapsindoors/map-template`.

## [1.91.5] - 2026-02-13

### Fixed

- Update Web-SDK version.

## [1.91.4] - 2026-02-11

### Fixed

- Removed demo-office monorepo package and dependencies

## [1.91.3] - 2026-02-11

### Added

- Client-generated session ID to session creation in `GeminiProvider`.
...
- Chat functionality with Gemini integration for natural language search and wayfinding.
- Location sharing consent popup for enhanced privacy control in chat interactions.
- "Ask with AI" button in search component and automatic map bounds fitting based on chat results.

### Fixed

- Improved null checks in setMinZoom, handleLocationClick, and other functions for better stability.
- Prevented message sending on Enter key during IME composition for better international keyboard support.
- Enhanced accessibility with aria-labels and improved cleanup for scroll behavior.
...
- Fixed residual user position state not being cleared when location becomes unavailable or on inactivity reset.

## [1.90.0] - 2026-01-15

### Added

- Possibility to remove Mapbox Extruded buildings via App Config.

## [1.89.10] - 2026-01-14

### Fixed

- Updated @mapsindoors/components dependency to include blue dot fixes: floor change handling, opacity management, and refresh logic improvements.
