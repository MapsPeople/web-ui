 ...
 */
function BottomSheet({ currentLocation, onClose, pushState, goBack, currentAppState, appStates }) {
    ...
    return <div ref={bottomSheetRef} className='bottom-sheets'>
        <ContainerContext.Provider value={bottomSheetRef}>
            <Sheet minHeight="128" isOpen={currentAppState === appStates.LOCATION_DETAILS}>
                <LocationDetails onStartWayfinding={() => pushState(appStates.WAYFINDING)} location={currentLocation} onClose={() => close()} />
            </Sheet>,
            <Sheet minHeight="220" isOpen={currentAppState === appStates.WAYFINDING}>
                <Wayfinding onStartDirections={() => pushState(appStates.DIRECTIONS)} onBack={() => goBack()} />
            </Sheet>
            <Sheet minHeight="220" isOpen={currentAppState === appStates.DIRECTIONS}>
